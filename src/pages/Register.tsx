...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) { setError('올바른 이메일 주소를 입력해주세요.'); return }
    if (password !== confirmPassword) { setError('비밀번호가 일치하지 않습니다.'); return }
    if (!/^(?=.*[a-zA-Z])(?=.*\d).{8,}$/.test(password)) { setError('비밀번호는 8자 이상, 영문과 숫자를 포함해야 합니다.'); return }
    setLoading(true)
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <div className="auth-field"><label htmlFor="password">비밀번호</label><input id="password" type="password" placeholder="8자 이상, 영문+숫자" value={password} onChange={e => setPassword(e.target.value)} required /></div>
