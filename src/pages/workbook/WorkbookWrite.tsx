...
export default function WorkbookWrite() {
  ...
  useEffect(() => { if (isEdit) loadWorkbook() }, [id])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) { const { error: err } = await updateWorkbook(id, { weekNumber: Number(form.week_number), title: form.title.trim(), content: form.content.trim(), fileUrl, isPublished: form.is_published }); if (err) throw err; alert('워크북이 수정되었습니다.') }
      else { const { error: err } = await createWorkbook({ weekNumber: Number(form.week_number), title: form.title.trim(), content: form.content.trim(), fileUrl, isPublished: form.is_published, authorId: user.id, authorName: user.user_metadata?.display_name || user.email }); if (err) throw err; alert('워크북이 등록되었습니다.') }
      navigate('/workbook')
    } catch (err: any) { setError(err.message || '오류가 발생했습니다.') } finally { setSubmitting(false) }
  }
  ...
  return (
    <>
      ...
      <section className="page-header"><div className="container"><h1>{isEdit ? '워크북 수정' : '워크북 등록'}</h1></div></section>
      <section className="section"><div className="container">
        <form className="lecture-write-form" onSubmit={handleSubmit}>
          ...
          <div className="lecture-form-row">
            ...
            <div className="lecture-form-group lecture-form-group-grow"><label>제목</label><input type="text" value={form.title} onChange={e => handleChange('title', e.target.value)} placeholder="워크북 제목을 입력하세요" required /></div>
          </div>
          <div className="lecture-form-group"><label>내용</label><textarea value={form.content} onChange={e => handleChange('content', e.target.value)} placeholder="실습 내용을 입력하세요" rows={12} /></div>
          ...
          <div className="lecture-form-actions"><Link to="/workbook" className="lecture-action-btn">취소</Link><button type="submit" className="lecture-action-btn primary" disabled={submitting}>{submitting ? '저장 중...' : (isEdit ? '수정' : '등록')}</button></div>
        </form>
      </div></section>
    </>
  )
